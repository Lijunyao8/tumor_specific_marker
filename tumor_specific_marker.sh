...
#CELL_TYPE=Plasma
perl /diskmnt/Datasets/mmy_scratch/lyao/MMY/Analysis/cell_surface_markers/Scripts/V8/automate_test/GTEX/step1.get_surface_genes_expression_data.pl ${OUT_DIR}/${CELL_TYPE}_specific_DEG_with_surface_annotations_from_3DB_gene_list.txt /diskmnt/Datasets/GTEX_tpm > ${OUT_DIR}/GTEX_expression_matrix_subset_tumor_cells_DEG.txt
grep captured ${OUT_DIR}/GTEX_expression_matrix_subset_tumor_cells_DEG.txt > ${OUT_DIR}/not_captured_GTEX_DEG.txt
...
/diskmnt/results/lyao/denali_lyao/LabCode_Tools/anaconda3/bin/perl /diskmnt/Datasets/mmy_scratch/lyao/MMY/Analysis/cell_surface_markers/Scripts/V8/automate_test/GTEX/step2.analyze_expression_change.pl ${OUT_DIR} > ${OUT_DIR}/GTEX_tissue_specific_DEG.txt
...
#############################################################
Rscript adding_tissue_specificity_annotations.R -o ${OUT_DIR} -c ${CELL_TYPE} -g ${GTEX_TT} -r ${HPA_RNA_TT} -p ${HPA_PRO_TT}
